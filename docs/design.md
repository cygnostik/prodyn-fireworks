...
Sound requires an initial explicit gesture. Hidden pages suspend visual time and audio, preserving the sound preference; returning resumes the previously enabled context without replaying missed sounds. Explicit mute persists, and a fresh page waits for a browser-permitted interaction before restoring a remembered sound-on choice. All important actions have DOM controls; the canvas is an additional direct-manipulation surface. Gentle mode reduces flashes but is not a guarantee of photosensitive safety.
